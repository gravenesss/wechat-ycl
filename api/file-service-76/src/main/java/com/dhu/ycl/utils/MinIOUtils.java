...
public class MinIOUtils {
    ...
    // 根据文件前置查询文件: 存储桶、前缀、是否使用递归查询。 返回 MinioItem 列表
    ...
    // 断点下载：存储桶、文件名称、起始字节的位置、长度。返回文件流
    ...
    // 获取路径下文件列表：存储桶、前缀、是否递归查找(false：模拟文件夹结构查找)。返回Item实体列表
    ...
    // 使用MultipartFile进行文件上传: 存储桶、文件、对象名、类型
    ...
    // 获取文件信息, 如果抛出异常则说明文件不存在。存储桶、文件名称
    ...
    // 获取文件外链：存储桶、文件名称、过期时间 <=7 秒 （外链有效时间（单位：秒））
